...
public:
	...
	template <class S>
	void get_partial_ranks(vector<S>& w, const string& method, uint num) const {
		if (num > sz) num = sz;
		vector<uint> tmp(sz);
		...
		if (method == "average") {
			for (uint c = 0, reps; c < num; c += reps) {
				reps = 1;
...
{
	ranker<T, lt<T> > r(v);
	r.get_partial_ranks(w, method, num);
}
