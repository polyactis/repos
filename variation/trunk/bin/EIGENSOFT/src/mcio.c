...
enum outputmodetype outputmode = ANCESTRYMAP ;
...
// basept + k + bigoff for individual ID 
 ...
 while (fgets(line, MAXSTR, fff) != NULL)  {
   ...
   if (skipit == NO) { 
    ...
    indiv = (int) iteml->data - (int) basept ;
    indiv -= bigoff ;
    val = atoi(spt[2]) ;
...
int snprawindex(SNPDATA **snpraw, int nreal, char *sname) 
{
    ...
    for (i=0; i<nreal; ++i)  {  
      if (strcmp(sname, snpraw[i] -> ID) == 0) return i ;
    }
...
{ 
 ...
 while (fgets(line, MAXSTR, fff) != NULL) { 
   ...
   if (skipit == NO) {  
    ...
    y = asum(gg, 3) ;  
    vst(gg, gg, 1.0/y, 3) ; 
    y = 0.5*(gg[1]+gg[2]) ;  /* est caucasian ancestry */ 
    indx -> thetatrue = y ;   
...
inpack(char *gname, SNP **snpm, Indiv **indiv, int numsnps, int numind) 
{
  ...
  if (hashcheck) {
   ...
   if (xihash != ihash) fatalx("OOPS indiv or snp file has changed since genotype file was created\n") ;
   if (xshash != shash) fatalx("OOPS indiv or snp file has changed since genotype file was created\n") ;
  }
...
*/
 ...
 if (maxgenolinelength == -1) 
   maxgenolinelength = maxlinelength(gname) ;
...
// load up packed genotype file for merge.
{
   ...
   rlen = nnint(ceil(y)) ;
   rlen = MAX(rlen, 48)  ;  
   ZALLOC(buff, rlen, char) ;
   ZALLOC(tbuff, rlen, char) ;
  ...
  sscanf(buff,"GENO %d %d %x %x", &xnind, &xnsnp, &xihash, &xshash) ;
