...
"""  
Examples:
	ValidateSNPRegion.py -i Desktop/ftRegions.csv -o /Network/Data/250k/tmp-yh/ftRegions_sig.tsv -u yh -p yh324
...
from variation.src.DrawSNPRegion import SNPPassingData
...
class ValidateSNPRegion(object):
	...
	def checkIfRegionSignificant(self, DrawSNPRegion_instance, snp_region, snp_info, analysis_method_id2gwr, value_criteria={1:8., 7:6.}):
		is_significant = 0
		analysis_method_id_set = Set(value_criteria.keys())
		for chr_pos in snp_region.chr_pos_ls:
			for analysis_method_id in analysis_method_id_set:
				gwr = analysis_method_id2gwr[analysis_method_id]
				data_obj = gwr.get_data_obj_by_chr_pos(chr_pos[0], chr_pos[1])
				if data_obj and data_obj.value>=value_criteria[analysis_method_id]:
					is_significant = 1
					break
	...
	def checkRegions(self, DrawSNPRegion_instance, snp_info, snp_region_ls, output_fname, value_criteria):
		...
		ft_phenotype_id_ls = self.get_phenotype_id_ls_given_biology_category_id(1)
