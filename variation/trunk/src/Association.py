...
Description:
	...
	But for emma, only phenotype data is allowed to have missing data.
"""
...
class Association(Kruskal_Wallis):
	...
	def __init__(self, **keywords):
		...
		self.output_results = {1:self.output_kw_results,
							...
							3:self.output_emma_results}
	def linear_model(cls, genotype_ls, phenotype_ls, min_data_point=3, snp_index=None):
		"""
		2008-11-10
		...
		if len(count_ls)>=2 and min(count_ls)>=min_data_point:	#require all alleles meet the min data point requirement
			...
			pdata = PassingData(snp_index=snp_index, pvalue=pvalue, count_ls=count_ls, var_perc=var_perc, coeff_list=coeff_list, coeff_p_value_list=coeff_p_value_list)
	...
	def Emma_whole_matrix(self, data_matrix, phenotype_ls, min_data_point=3):
		...
		rs = rpy.r.emma_REML_t(non_NA_phenotype_ar.transpose(), new_data_matrix.transpose(), kinship_matrix)	#in emma, row is marker. col is strain.
