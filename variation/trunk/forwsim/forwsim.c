...
#define      SSIZE           1000    // Number of samples to output from the final population 
#define    POPSIZE           1000    // Total number of chromsomes in the diploid population
#define  SEQLENGTH         125000    // Length of the DNA sequence in basepairs. Choose a large value >= DELETE*mu*POPSIZE so that you don't run out of locations under the infinite-sites model.
#define         re           0.25    // Per-generation per-sequence rate of recombination
#define         mu           0.25    // Per-generation per-sequence rate of mutation//0.019607843//0.13333333//0.018181818    
...
main(){ 
rec = 1 - exp(-re);for(i=0;i<length;i++){init[i]=time(NULL);}mt.seed(init,length);
...
for(m=0;m<POPSIZE;m++){
if(indic[m]==0 || (i%DELETE>=DELETE-10 || i%DELETE==0 || i>=GEN-10)){
p = gen1->np[m];j = gen1->n[m];if(edisp[j]==j){jj=j;}else{jj=edisp[j];}
if(p<rec){if(j%2==0){k = j+1;}if(j%2==1){k = j-1;}if(edisp[k]==k){kk=k;}else{kk=edisp[k];}++recount[jj];++recount[kk];}
else{
if(current[jj]!=jj){
if(mcount[m]!=0){memcpy(&odd[m].chr->q[0],&odd[current[jj]-POPSIZE].chr->q[0],4*siz[j]);}
else{odisp[m]=current[jj]-POPSIZE;}size[m]=siz[j];}
if(current[jj]==jj){
...
if(p<rec){
r = int((SEQLENGTH-1)*drand());if(j%2==0){k = j+1;}if(j%2==1){k = j-1;}
if(edisp[j]==j){jj=j;}else{jj=edisp[j];}if(edisp[k]==k){kk=k;}else{kk=edisp[k];}--recount[jj];--recount[kk];
...
if(r >=even[jj].chr->q[trk]){start=trk;trk=(start+end)/2;if(trk==start){++trk;}continue;}
...
if(r >=even[kk].chr->q[trk]){start=trk;trk=(start+end)/2;if(trk==start){++trk;}continue;}
...
for(m=0;m<POPSIZE;m++){
if(indic[m]==0 || (i%DELETE>=DELETE-10 || i%DELETE==0 || i>=GEN-10)){
p = gen1->np[m];j = gen1->n[m];if(odisp[j]==j){jj=j;}else{jj=odisp[j];}
if(p<rec){if(j%2==0){k = j+1;}else{k = j-1;}if(odisp[k]==k){kk=k;}else{kk=odisp[k];}++recount[jj];++recount[kk];}
else{
if(current[jj]!=jj){
if(mcount[m]!=0){memcpy(&even[m].chr->q[0],&even[current[jj]-POPSIZE].chr->q[0],4*size[j]);}
else{edisp[m]=current[jj]-POPSIZE;}siz[m]=size[j];}
if(current[jj]==jj){
...
if(p<rec){
r = int((SEQLENGTH-1)*drand());if(j%2==0){k = j+1;}if(j%2==1){k = j-1;}  
if(odisp[j]==j){jj=j;}else{jj=odisp[j];}if(odisp[k]==k){kk=k;}else{kk=odisp[k];}--recount[jj];--recount[kk];
...
if(r >=odd[jj].chr->q[trk]){start=trk;trk=(start+end)/2;if(trk==start){++trk;}continue;}
...
if(r >=odd[kk].chr->q[trk]){start=trk;trk=(start+end)/2;if(trk==start){++trk;}continue;}
...
for(m=0;m<SSIZE;m++){
for(l=0;l<siz[m];l++){output1<<even[m].chr->q[l]<<" ";}output1<<"\n";}
output1.close();
}
