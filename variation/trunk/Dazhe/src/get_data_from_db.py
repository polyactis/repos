...
def _run_():   
    ...
    analysis_method_dict={"1":"Wilcoxon","7":"Emma"}
    ...
    while(1):
        ...
        if int(row[1]) not in WantedTracks:
	    continue
	sql_gb = "select tag from typelist where tag='"+"_".join(row[:3])+":"+source_name+"'"
        ...
        if exist_or_not == 0: # Data not in database yet -)
            ...
            mk_conf("_".join(row[:3]), phenotype_method_dict[row[1]]+" Using "+analysis_method_dict[row[2]], max_score, min_score)
    cursor_250k.close ()
    conn_250k.close ()
...
link            = sub {
                   ...
                   return sprintf("http://cypress.usc.edu/SNP/?chromosome=%d&position=%d&call_method_id=%d&phenotype_method_id=%d&analysis_method_id=%d&score=%.6f",$snp[0],$snp[1],$met[0],$met[1],$met[2],$score);
		}
""")
