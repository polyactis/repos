...
class DisplaytopsnptestrmController(BaseController):
	...
	def type(self, id=None):
		CandidateGeneTopSNPTestRMType = model.Stock_250kDB.CandidateGeneTopSNPTestRMType
		CandidateGeneTopSNPTestRM = model.Stock_250kDB.CandidateGeneTopSNPTestRM
		...
		extra_condition = 'c.type_id =%s and s.id=c.results_id'%id
		
		c.phenotype_info  = h.getPhenotypeInfo(ResultsMethod.table.name, extra_condition, extra_tables)
		extra_condition = 's.type_id =%s'%id
		
		c.list_info = h.getListTypeInfo(CandidateGeneTopSNPTestRM.table.name, extra_condition)
		...
		c.type = CandidateGeneTopSNPTestRMType.get(id)
		...
		return render('/display_top_snp_test_rm_type.html')
