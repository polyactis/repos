...
class TestSetup(OptiluxPolicyTestCase):
    ...
    def test_activity_report_action_installed(self):
        self.failUnless('activity-report' in self.portal.portal_actions.site_actions.objectIds())
    ...
    def test_manage_own_portlets_permission(self):
        permissions = [r['name'] for r in 
                        self.portal.rolesOfPermission('Portlets: Manage own portlets') if r['selected']]
        self.failUnless('StaffMember' in permissions)
        self.failIf('Member' in permissions)

    def test_add_portal_members_permission(self):
        permissions = [r['name'] for r in 
                        self.portal.rolesOfPermission('Add portal member') if r['selected']]
