...
bool CCELFileData::ReadXDABCel(bool bReadHeaderOnly)
{
...
#else
	boost_io::filtering_istream instr;
	if ((strstr(m_FileName.c_str(), ".gz") != NULL) || 
	    (strstr(m_FileName.c_str(), ".GZ") != NULL))
	  {
	    instr.push(boost_io::gzip_decompressor());
	  }
	else if ((strstr(m_FileName.c_str(), ".bz2") != NULL) || 
		 (strstr(m_FileName.c_str(), ".BZ2") != NULL))
	  {
	    instr.push(boost_io::bzip2_decompressor());
	  }
        instr.push(boost_io::file_source(m_FileName.c_str(), std::ios::in | std::ios::binary));
	// filter throws exception on problem instead of returning NULL
#endif
	...
	if (bReadHeaderOnly)
	  { // keep going if it is not just header - close later
#ifndef HAVE_BOOST
	    //boost::filtering_istream doesn't have a close method
	    instr.close();
#else
	    instr.reset();
#endif
  ...
  // FIXME: USE_MEM_MAPPING should be set in all the makefiles?
	...
	//HAVE_BOOST defines _DONT_USE_MEM_MAPPING_, so no instr.reset() here
	instr.close(); // memory map code doesn't use the ifstream
  ...
  /* We are already at the right place in the ifstream,
     just keep reading - no need to seek */
  // skip the header
  // fseek(m_File,iHeaderBytes,SEEK_SET);
  // printf("fpos=%u",ftell(m_File)); // debug
  // read the entire file into memory...
  // calculating the datasize from the header, since it may be a compressed stream
  int32_t datasize = m_HeaderData.GetCells() * (FLOAT_SIZE + FLOAT_SIZE + SHORT_SIZE)
    + m_HeaderData.GetMasked() * 2 * SHORT_SIZE
    + m_HeaderData.GetOutliers() * 2 * SHORT_SIZE;
  m_lpData = new char[datasize];
  instr.read(m_lpData,datasize);
  // TODO - need to catch exception here
  if (instr.gcount() != datasize) {
