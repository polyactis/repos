...
while (my $current_line = <$full_list>) {
	...
	print $outfile join("\t", ("AFFYID", "RSID", "pos", "AlleleA", "AlleleB"));
	
	@nspfiles = ();
	@styfiles = ();

	foreach my $nspdir (@NSPdirs) {
	    push @nspfiles, (IO::File->new($nspdir . "/NSP_chromosome_${chrom}.txt", "r"));
	}
	foreach my $stydir (@STYdirs) {
	    push @styfiles, (IO::File->new($stydir . "/STY_chromosome_${chrom}.txt", "r"));
	}
