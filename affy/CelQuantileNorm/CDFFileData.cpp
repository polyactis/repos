...
bool CCDFFileData::ReadProbeSetNames()
{
	// Read the probe set names, close if failed.
...
bool CCDFFileData::Open(bool bReadHeaderOnly, bool bReadProbeSetNamesOnly)
{
	// First close the file.
	Close();

	if(bReadProbeSetNamesOnly == true)
	  {
	    //Reading probe set names requires reading beyond the header
	...
	{
#if defined(_USE_MEM_MAPPING_)
		return ReadXDAFormatUsingMemMap(bReadHeaderOnly, bReadProbeSetNamesOnly);
#else
		return ReadXDAFormat(bReadHeaderOnly, bReadProbeSetNamesOnly);
#endif
	}
	else
...
bool CCDFFileData::ReadXDAFormatUsingMemMap(bool bReadHeaderOnly, bool bReadProbeSetNamesOnly)
{
	// Open the file.
	std::ifstream instr;
	...
	// Stop if just reading the probe set names.
	if (bReadProbeSetNamesOnly)
		return true;
...
bool CCDFFileData::ReadXDAFormat(bool bReadHeaderOnly, bool bReadProbeSetNamesOnly)
{
	// Open the file.
	std::ifstream instr;
	instr.open(m_FileName.c_str(), std::ios::in | std::ios::binary);
