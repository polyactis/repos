...
//
////////////////////////////////////////////////////////////////
#include "SAXArrayHandlersTest.h"
...
void SAXArrayHandlersTest::testCreation_startElement_with_bad_data()
{
	...
	{
	public:
		unsigned int getLength(void) const { return 2; }
        const XMLCh *getName(const unsigned int) const { return NULL; }
        const XMLCh *getType(const XMLCh *const ) const { return NULL; }
        const XMLCh *getType(const unsigned int) const { return NULL; }
        const XMLCh *getValue(const char *const ) const { return NULL; }
        const XMLCh *getValue(const XMLCh *const ) const { return NULL; }
        const XMLCh *getValue(const unsigned int) const { return NULL; }
	};
	ArrayData array;
	SAXArrayHandlers handler(&array);
