...
STDMETHODIMP CProbeSetMultiDataExpressionDataCOM::InterfaceSupportsErrorInfo(REFIID riid)
{
	static const IID* arr[] = 
	{
		&IID_IProbeSetMultiDataExpressionData
	};
...
STDMETHODIMP CProbeSetMultiDataExpressionDataCOM::get_name(BSTR* pVal)
{
	*pVal = COMStringUtils::ConvertString(data.name);
	return S_OK;
}

STDMETHODIMP CProbeSetMultiDataExpressionDataCOM::get_quantification(float* pVal)
{
	*pVal = data.quantification;
    return S_OK;
}

STDMETHODIMP CProbeSetMultiDataExpressionDataCOM::get_metricCount(int *pVal)
{
    *pVal = (int)data.metrics.size();
    return S_OK;
}

STDMETHODIMP CProbeSetMultiDataExpressionDataCOM::GetMetricName(int index, BSTR* pVal)
{
...
STDMETHODIMP CProbeSetMultiDataExpressionDataCOM::GetMetric(int index, VARIANT* pVal)
{
    ParameterNameValueType &nv = data.metrics[index];
