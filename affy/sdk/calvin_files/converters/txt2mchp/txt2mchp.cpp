...
\page description Writing genotype results to a "multi-data" CHP file.
 ...
 * @param outFile The name of the output CHP file.
 * @param maxProbeSetNameLength The maximum length of the probe set names.
 * @param data The data from the input file.
 * @param extraColTypes The types (float, int, ubyte) of the extra columns.
 */
static void AddFileBody(const string &outFile, int maxProbeSetNameLength, const list<list<string>> &data, const vector<string>& extraColTypes)
{
	...
	for (list<list<string>>::const_iterator rowIt=data.begin(); rowIt!=data.end(); rowIt++)
	{
		list<string>::const_iterator colIt=rowIt->begin();
		entry.name = *colIt;
		++colIt;
		...
		entry.metrics.resize(rowIt->size() - 3);
		int colIdx = 0;
		for (; colIt!=rowIt->end(); colIt++, colIdx++)
		{
 ...
 * @param maxProbeSetNameLength The maximum length of the probe set names.
 */
static void ReadData(const string &inFile, list<list<string>> &data, int &maxProbeSetNameLength)
{
	// Open the file
...
int main(int argc, char * argv[])
{
	try
	{
		...
		ReadData(inFile, data, maxProbeSetNameLength);

		// Create the CHP file.
		CreateFileWithHeader(execId, celFile, outFile, colNames, colTypes, (int) data.size(), maxProbeSetNameLength,
		...
		AddFileBody(outFile, maxProbeSetNameLength, data, colTypes);
	}
	catch (string s)
	{
		cout << s << endl;
